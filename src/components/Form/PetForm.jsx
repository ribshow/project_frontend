...
import stylesForm from "./pet-form.module.css";
...
const PetForm = ({ handleSubmit, petData, btnText }) => {
  ...
  function handleSituation(e) {
    setPet({
      ...pet,
      situation: e.target.options[e.target.selectedIndex].text,
    });
  }
  ...
  return (
    <div>
      <form className={stylesForm.container} onSubmit={submit}>
        <div>
          {preview.length > 0
            ? preview.map((image, index) => (
                <img
                  className={stylesForm.img_container}
            ...
            : pet.image &&
              pet.images.map((image, index) => (
                <img
                  className={stylesForm.img_container}
                  src={`${process.env.REACT_APP_API}/public/images/pets/${image.name}`}
                  alt={pet.name}
                  key={index}
                />
              ))}
        ...
        <Input
          ...
          handleOnChange={handleChange}
        />
        <input className={stylesForm.btnSubmit} type="submit" value={btnText} />
      </form>
    </div>
  );
};
